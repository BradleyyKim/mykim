...
The AI wrote the code. What I did was set the constraints, and for this tool there were three.

First, no dependencies. The moment `pip install` is required, this tool stops opening six months from now. That is why the import list ends where it does.
...
Second, no cache. From the moment I can't be sure how old the screen in front of me is, I stop looking at it.

```python
self.send_header("Cache-Control", "no-store")
```

Sweeping on every request meant the sweep had to be cheap. The third constraint came from there.
...
Slicing `dirnames[:]` in place is the whole trick. Build a new list and rebind the name instead, and `os.walk` never learns about the pruning and descends into `node_modules` anyway.

I was curious, so I dropped the pruning and swept the same two folders. Two tenths of a second became 9.5 seconds, and the md count went from 477 to 16,481. Those extra sixteen thousand are mostly READMEs inside other people's packages.

Reading the code I got back, that is the place I checked. Not what had been built, but which line my constraint survived in.
