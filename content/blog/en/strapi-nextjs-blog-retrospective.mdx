...
Checking during the migration, the function names and signatures stayed put and only the bodies changed.
...
The `fetch` prefix exists because this was once an HTTP call, and `async` survives for the same reason. It reads the filesystem synchronously now, and because the name never changed, not one caller had to be touched.

Since then, whenever I ask an AI to work in this repo, I explain the three layers first. Skip the explanation and back comes a suggestion to strip the "unnecessary wrapping" — which is precisely the wrapping that absorbed the migration cost. Without being told why something was kept, it looks like cleanup.
