...
export default function SocialIcon({ type, size = 20, className = "" }: SocialIconProps) {
  ...
  if (type === "rss") {
    return (
      <Rss
        className={`text-orange-500 group-hover:text-orange-600 ${className}`}
  ...
  if (type === "email") {
    return (
      <Mail
        className={`text-gray-600 dark:text-gray-400 group-hover:text-gray-800 dark:group-hover:text-gray-200 ${className}`}
        style={{ width: size, height: size }}
      />
    );
  }
  ...
  if (type === "instagram") {
    return (
      <Instagram
        className={`text-pink-500 group-hover:text-pink-600 ${className}`}
        style={{ width: size, height: size }}
      />
    );
  }
  ...
  return (
    <Image
      ...
      width={size}
      height={size}
      className={`object-contain transition-opacity duration-200 ${icon.hoverEffect} ${className}`}
    />
  );
}
